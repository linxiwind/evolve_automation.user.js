...
// ==/UserScript==
//
// This script forked from Vollch's script. Original script: https://gist.github.com/Vollch/b1a5eec305558a48b7f4575d317d7dd1
//
...
(function($) {
    ...
    const prestigeNames = {mad: "核爆重置", bioseed: "播种重置", cataclysm: "大灾变重置", vacuum: "真空坍缩", whitehole: "黑洞重置", apocalypse: "人工智能觉醒", ascension: "飞升重置", demonic: "恶魔灌注", terraform: "Terraform"};
    ...
    function getTooltipInfo(obj) {
        ...
        if ((obj instanceof Technology || (!settings.autoARPA && obj._tab === "arpa") || (!settings.autoBuild && obj._tab !== "arpa")) && !state.queuedTargetsAll.includes(obj) && !state.triggerTargets.includes(obj)) {
            ...
            if (conflict) {
                notes.push(`与${conflict.obj.name}因${conflict.res.name}而冲突 (${conflict.obj.cause})`);
            }
        }
    ...
    const checkTypes = {
        ...
        BuildingQueued: { fn: (b) => state.queuedTargetsAll.includes(buildingIds[b]), ...argType.building, desc: "如果建筑已在队列中，则返回真值", title:"建筑是否在队列" },
        ...
        Other: { fn: (o) => argMap.other(o), ...argType.other, desc: "其他未分类的变量", title:"其它" },
    }
    ...
    function updateGeneralSettingsContent() {
        ...
        addSettingsToggle(currentNode, "tickSchedule", "脚本刻度排程", "启用后，脚本会将其时间安排在游戏时间之后运行，而不是同时执行两个任务。分割长的任务允许浏览器在游戏和脚本运行之间更新用户界面，使游戏运行更流畅，但对节流的保护较差。这也会导致一些奇怪的错误，比如突然的跳动，或者对同一游戏的跳动进行双重处理。不稳定的实验性选项，使用风险自负。");
    ...
    function updateEvolutionSettingsContent() {
        ...
        addSettingsSelect(currentNode, "userPlanetTargetName", "欲选择的星球", "在特定重置后自动选择相应的星球。警告! 脚本忽略了G.E.C.K.所做的改变，你需要在使用它之后手动选择星球。", planetOptions);
    ...
    function updateFleetOuter(currentNode, secondaryPrefix) {
        ...
        addSettingsToggle(currentNode, "fleetScanEris", "扫描Eris", "派遣带有量子扫描器的单一护卫舰前往Eris，忽略区域权重。");
    ...
    function updateStorageSettingsContent() {
        ...
        addSettingsToggle(currentNode, "storageAssignExtra", "是否分配缓冲用的存储", "以超过需要数值的3%进行分配，以保证能达到所需要的数值，即使脚本的其他部分试图卖出\\弹出\\切换生产等等。当启用手动交易时，应用额外的调整，从销售阈值中扣除。");
        addSettingsToggle(currentNode, "storageAssignPart", "分配部分存储", "当启用时，脚本将被允许分配一些板条箱和容器，即使产生的存储空间不足以建造新的建筑物。它允许预先建立资源储备以备不时之需，但也有可能会造成危险。\n 如果脚本不允许重新分配非空的存储空间，那么当存储的资源无法使用时，它就会将存储空间锁定在某个位置。\n 如果允许脚本重新分配非空的存储空间，它可能会浪费生产材料的时间，而这些材料可能需要被处理掉。");
    ...
    function updateProductionTableFoundry(currentNode) {
        ...
        addSettingsToggle(currentNode, "jobDisableCraftsmans", "在可能的情况下手工制作", "在允许手工制作的情况下禁用工匠");

        currentNode.append(`
          <table style="width:100%">
            <tr>
              <th class="has-text-warning" style="width:21%" title="Resource name">资源名称</th>
              <th class="has-text-warning" style="width:17%" title="Resource won't ever be crafted with this option disabled">是否启用</th>
              <th class="has-text-warning" style="width:17%" title="在禁用此选项的情况下，不会使用工匠制造资源。">工匠</th>
        ...
        currentNode.append(`
          <table style="width:100%">
            <tr>
              ...
              <th class="has-text-warning" style="width:9%" title="启用后，脚本将限制分配给工人的数量，使其达到最大的有用数量，将闲置的工人转移到其他工作上。">智能</th>
        ...
        if (scriptNode.length === 0) {
            ...
            $('#resources').append(`
              <div id="autoScriptContainer" style="margin-top: 10px;">
                <h3 id="toggleSettingsCollapsed" class="script-collapsible text-center has-text-success">自动开关</h3>
            ...
            createSettingToggle(togglesNode, 'autoPrestige', '自动威望重置', '允许脚本在达到设定的目标后完成当前的运行。即使是手动重置，也建议设置威望重置类型，因为脚本会利用威望重置类型做出各种决定，比如选择神学技术，或者跳过通往错误方向的建筑。');
            ...
            createSettingToggle(togglesNode, 'autoGovernment', '自动政府', '管理政府和总督的变化，当他们变得可用时。总督将被选择一次，并且不会被重新分配，除非是手动解雇。');
            ...
            createSettingToggle(togglesNode, 'autoAssembleGene', '自动组装基因', '当知识满了以后，自动进行基因重组。进行首次测序时不生效。');
            ...
            togglesNode.append('<a class="button is-dark is-small" id="bulk-sell"><span>批量销售</span></a>');
